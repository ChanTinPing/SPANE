...
def parser_add_specific_args(parser, method):
    ...
    if method == 'SPANE':
        ...
        parser.add_argument('--PM_num', type=int, required=True)
    else:
...
if __name__ == "__main__":
    ...
    # Store results 
    print(f"SPANE PM_num: {sp.PM_num}, Trial_id: {sp.trial_id}, Result: {mean_wts}")
    
    with open(f'result/flex/result.txt', 'a') as file:
        file.write(f'{sp.method} PM_num: {sp.PM_num}, Trial_id: {sp.trial_id}, Result: {mean_wts} \n')
    result_dir = f'result/flex/{sp.PM_num}'
