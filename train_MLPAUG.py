...
def generate_random_permutations(N, num):
    ''' generate num random permutations, include identity '''
...
def transform_state_action(trans: tuple[int, ...], obs, action, next_obs):
    ''' return after transformation obs, action, next_obs '''
...
def obj(sp: dict):
    ...
    args.run_no = 0  # Number of data collections with the environment in each epoch
        ...
        while not done:
            ...
            if not is_valid:
                ...
                for trans in transformation_groups: 
                    ...
                    memory.push(t_obs, feat, t_action, reward, t_next_obs, next_feat, done)
            state = state2
    ...
    # 4. Pre-collect data
    for _ in range(int(100 + (args.transform_num-1) / args.transform_num)):
